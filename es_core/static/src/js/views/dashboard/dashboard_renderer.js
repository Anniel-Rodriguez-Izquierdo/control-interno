odoo.define('es_core.DashboardRenderer', function (require) {
"use strict";
    ...
    var DashboardRenderer = FormRenderer.extend({
        ...
        _addStatisticTooltip: function ($el, node) {
            $el.tooltip({
                ...
                title: function () {
                    return QWeb.render('es_core.StatisticTooltip', {
                        debug: config.debug,
                        node: node,
                    });
                }
        ...
        _renderView: function () {
            ...
            return r.then(function () {
                ...
                // Aplicando el efecto del layout Gridster cuando toda la vista se ha renderizado
                var $gridster = self.$el.find(".gridster ul").gridster({
        ...
        /**
         */
        _renderTagGridster: function (node) {
            var self = this;
            var $container = $('<div>', {class: 'gridster'});
            var $ul = $('<ul>');
            _.each(node.children, function (child) {
                $ul.append(self._renderTagGitem(child));
            });
            $container.append($ul);
            return $container;
        },
        ...
        /**
         */
        _renderTagGitem: function (node) {
            var self = this;
            var $li = $('<li>').attr('data-row', node.attrs.row)
                            .attr('data-col', node.attrs.col)
                            .attr('data-sizex', node.attrs.sizex)
                            .attr('data-sizey', node.attrs.sizey);

            _.each(node.children, function (child) {
                $li.append(self._renderNode(child));
            });
